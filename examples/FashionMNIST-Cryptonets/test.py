...
def test_mnist_cnn(FLAGS):
    ...
    ## either this (restore model):
    with tf.gfile.GFile('graph_optimized.pb', 'rb') as f:
        graph_def_optimized = tf.GraphDef()
        graph_def_optimized.ParseFromString(f.read())
    G = tf.Graph()

    # ## or this (load from .txt files):
    # x = tf.placeholder(tf.float32, [None, 784])#, name='inx:0')
    # y_ = tf.placeholder(tf.float32, [None, 10])#, name='iny:0')
    # y_conv = common.cryptonets_model(x, 'test')

    with tf.Session(graph=G) as sess:
    # with tf.Session() as sess:

        ## if restore:
        y_conv = tf.import_graph_def(graph_def_optimized, return_elements=['logits_out:0'])[0]
        print('Operations in Optimized Graph:')
        print([op.name for op in G.get_operations()])
        x = G.get_tensor_by_name('import/inx:0')
        y_ = G.get_tensor_by_name('import/iny:0')
